...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  ##############################################################################
  fwd=x.dot(Wx)+prev_h.dot(Wh)+b
  next_h=np.tanh(fwd)

  cache=x,prev_h,Wx,Wh,fwd
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  x, prev_h, Wx, Wh,fwd = cache

    # Backprop step 2:
  dfwd = (1 - np.tanh(fwd)**2) * dnext_h

    # Backprop step 1:
  dx = np.dot(dfwd, Wx.T)
  dWx = np.dot(x.T, dfwd)
  dprev_h = np.dot(dfwd, Wh.T)
  dWh = np.dot(prev_h.T, dfwd)
  db = np.sum(dfwd, axis=0)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    if t > 0:
      prev_h = h[t - 1]
      prev_c = next_c
    h[t], next_c, cache_next = lstm_step_forward(x[t], prev_h, prev_c, Wx, Wh, b)
    cache.append(cache_next)
  h = h.transpose(1, 0, 2)
  ##############################################################################
  #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    dh_current = dh[t] + dh_prev
    dc_current = dc_prev
    dx_t, dh_prev, dc_prev, dWx_t, dWh_t, db_t = lstm_step_backward(dh_current, dc_current, cache[t])
    dx[t] += dx_t
    dh0 = dh_prev
    dWx += dWx_t
    dWh += dWh_t
